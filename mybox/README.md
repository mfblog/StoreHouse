...
### v1.1.0 (2025-09-10)
**MosDNS 配置管理重大更新**
- 🚀 **配置文件管理**：完整的 MosDNS sub_config 配置文件 CRUD 操作
- ⚙️ **高级功能选项卡**：重新组织界面，将配置管理设为高级功能
- 🔒 **智能权限控制**：基于服务安装状态的访问控制和友好提示
- 📝 **实时编辑器**：支持 YAML 配置文件的在线编辑、验证和保存
- 🛡️ **安全机制**：路径验证、自动备份、双重权限验证
- 🎯 **自动加载**：智能检测服务状态，自动加载配置文件列表
- 📁 **目录自定义**：支持自定义配置文件目录扫描
- 🎨 **现代界面**：选项卡式设计，基础功能与高级功能分离

### v1.0.7 (2025-01-09)
**配置编辑器优化版本**
- 🎯 **分级显示优化**：将"大纲"重命名为"分级显示"，更直观的功能描述
- 📍 **界面布局调整**：分级显示按钮移至配置文件路径后方，布局更紧凑
- 🔄 **自动加载改进**：进入配置编辑页面时自动加载配置文件，无需手动操作
- 📊 **默认分级展示**：分级面板默认展开，提供更好的用户体验
- 🎨 **响应式布局**：关闭分级面板时编辑器自动充满屏幕，最大化编辑空间
- 🚀 **加载速度优化**：移除不必要的弹窗提示，提升加载响应速度
- 🔧 **多重保障机制**：确保配置在各种场景下都能正常自动加载
...
**高级功能**:
- 配置文件管理：YAML 配置文件的 CRUD 操作
- 实时编辑器：在线编辑、验证、保存配置
- 目录扫描：自定义配置文件目录
- 智能权限控制：基于服务安装状态的访问管理
- 自动备份：配置修改前自动创建备份
...
### v1.1.0 (2025-09-10)
- ✅ 新增 MosDNS 配置文件完整管理功能
- ✅ 实现高级功能选项卡，重新组织界面结构
- ✅ 添加智能权限控制和安全访问机制
- ✅ 支持 YAML 配置文件在线编辑、验证、保存
- ✅ 实现自动备份和路径安全验证
- ✅ 优化用户体验，支持自动加载和目录自定义

### v1.0.7 (2025-01-09)
- ✅ 优化配置编辑器用户体验，重命名"大纲"为"分级显示"
- ✅ 调整界面布局，分级显示按钮移至配置文件路径后方
- ✅ 实现自动加载配置文件，默认展开分级面板
- ✅ 完善响应式布局，关闭分级面板时编辑器充满屏幕
- ✅ 提升加载性能，移除阻塞性弹窗提示
